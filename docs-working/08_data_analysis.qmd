...
```{r raw-outputs}
total_ends <- qread(file.path(output_model_farm_path, "total_ends.qs")) %>% 
  mutate(
    feed_group = as.factor(str_remove(feed, "_min|_max")),
    feed_type = case_when(
    ...
    sd = set_units(sd, "g"),
    mean_biomass = set_units(mean, "g g_fish-1"),
    sd_biomass = set_units(sd, "g g_fish-1")
  )

cohort_results <- qread(file.path(output_model_cohort_path, "cohort_results.qs")) %>% 
  mutate(
    feed_group = as.factor(str_remove(feed, "_min|_max")),
    feed_type = case_when(
      str_detect(feed, "_min") ~ "min",
...
```{r feed-prov-lost}
#| code-summary: Show differences between diets in feed provided and lost
#| fig-cap: Mean food provided and uneaten feed lost to the environment (g/g salmon)
...
ggplot(
    feed_end, 
    aes(x = feed_group, y = mean_per_biom, fill = feed_type, group = feed)
  ) +
  geom_boxplot() +
