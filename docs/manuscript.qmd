...
|:------:|:------:|:-----------:|
| $\alpha$             | `r unname(params['alpha'])`      | [@broekhuizen_modelling_1994] |
| $\epsilon_{P}$       | `r unname(params['epsprot'])`    | @baldan_r_2018 |
| $\epsilon_{L}$       | `r unname(params['epslip'])`     | @baldan_r_2018 |
| $\epsilon_{C}$       | `r unname(params['epscarb'])`    | @baldan_r_2018 |
| $\epsilon_{O_2}$     | `r unname(params['epsO2'])`      | @elliott_energy_1975 |
| $p_k$                | `r unname(params['pk'])`         | @macnaughton_using_2019 |
| $k_0$                | `r unname(params['k0'])`         | @macnaughton_using_2019 |
| $m$                  | `r unname(params['m'])`          | @broekhuizen_modelling_1994 | 
| $n$                  | `r unname(params['n'])`          | @baldan_r_2018 |
| $\beta C$            | `r unname(params['betac'])`      | @jonsson_thermal_2001 |
| $T_{max}$            | `r unname(params['Tma'])`        | @poletto_unusual_2017 | 
| $T_{opt}$            | `r unname(params['Toa'])`        | @jonsson_thermal_2001 | 
| $T_{min}$            | `r unname(params['Taa'])`        | @vadboncoeur_lowering_2023 | 
| $\omega$             | `r unname(params['omega'])`      | @baldan_r_2018 |
| $a$                  | `r unname(params['a'])`          | @van_tien_development_2016 |
| $k$                  | `r unname(params['k'])`          | @van_tien_development_2016 | 
| $eff$                | `r unname(params['eff'])`        | @uglem_does_2020 | 
| $\overline{W}$       | `r unname(params['meanW'])`      | @dempsey_estimating_2023 | 
| $\Delta{W}$          | `r unname(params['deltaW'])`     | @dempsey_estimating_2023 | |
| $\overline{I_{max}}$ | `r unname(params['meanImax'])`   | @lall_nutrition_2009 | 
| $\Delta{I_{max}}$    | `r unname(params['deltaImax'])`  | @lall_nutrition_2009 | 
| $\overline{F}$       | `r unname(params['overFmean'])`  | |
| $\Delta{F}$          | `r unname(params['overFdelta'])` | |
| $mort$               | `r unname(params['mortmyt'])`    | @tvete_towards_2023 |
...
Total uneaten feed across a cohort (548 days) fed the reference feed ranged from 
`r uneaten_total_stats$min[uneaten_total_stats$feed == "reference"]` to 
`r uneaten_total_stats$max[uneaten_total_stats$feed == "reference"]` g kg$^{-1}$ fish, with a mean of 
`r uneaten_total_stats$mean[uneaten_total_stats$feed == "reference"]` g kg$^{-1}$ fish. 
Differences between the feeds were very small, but slightly less feed was left uneaten in the reference scenario (@fig-total-uneaten-perday). Uneaten feed in the past scenario ranged from 
`r uneaten_total_stats$min[uneaten_total_stats$feed == "past"]` to 
`r uneaten_total_stats$max[uneaten_total_stats$feed == "past"]` g kg$^{-1}$ fish (mean of 
`r uneaten_total_stats$mean[uneaten_total_stats$feed == "past"]` g kg$^{-1}$ fish) while uneaten feed in the future scenario ranged from 
`r uneaten_total_stats$min[uneaten_total_stats$feed == "future"]` to 
`r uneaten_total_stats$max[uneaten_total_stats$feed == "future"]` g kg$^{-1}$ (mean of 
`r uneaten_total_stats$mean[uneaten_total_stats$feed == "future"]`  g kg$^{-1}$ fish).
...
The composition of the uneaten feed is (obviously) the same as the total feed composition detailed in (@sec-feeds).
...
Overall, total excretion was lowest when when fish were fed the past feed and highest when fish were fed the reference feed (@fig-total-excreted-perday). 
Total excreted faeces across a cohort (548 days) in the reference scenario ranged from `r excreted_total_stats$min[excreted_total_stats$feed == "reference"]` to 
`r excreted_total_stats$max[excreted_total_stats$feed == "reference"]` g kg$^{-1}$ fish d$^{-1}$, with a mean of 
`r excreted_total_stats$mean[excreted_total_stats$feed == "reference"]`.
Total excreted faeces across a cohort in the past scenario ranged from 
`r excreted_total_stats$min[excreted_total_stats$feed == "past"]` to 
`r excreted_total_stats$max[excreted_total_stats$feed == "past"]` g kg$^{-1}$ fish d$^{-1}$ (mean of 
`r excreted_total_stats$mean[excreted_total_stats$feed == "past"]` g kg$^{-1}$ fish d$^{-1}$).
Total excreted faeces across a cohort in the future scenario ranged from 
`r excreted_total_stats$min[excreted_total_stats$feed == "future"]` to 
`r excreted_total_stats$max[excreted_total_stats$feed == "future"]` g kg$^{-1}$ fish d$^{-1}$ (mean of 
`r excreted_total_stats$mean[excreted_total_stats$feed == "future"]` g kg$^{-1}$ fish d$^{-1}$). 
