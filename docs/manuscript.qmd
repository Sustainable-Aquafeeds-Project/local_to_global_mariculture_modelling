...
| $p_k$                | `r unname(params['pk'])`         | d$^{-1}$                 | Temperature coefficient for the fasting catabolism | @macnaughton_using_2019 |
...
[Plan to put full details of ingredients in a supplementary section. But possibly report these stats in Results under Uneaten feed?]{.aside}
...
Ingredients are tracked seperately through the model with their own digestibility coefficients. Overall, proteins within the reference feed were 
`r round(100*filter(feed_summary, feed == "reference", macro == "P")$digest, 1)`% digestible, proteins within the past feed were 
`r round(100*filter(feed_summary, feed == "past", macro == "P")$digest, 1)`% digestible, and proteins within the future feed were 
`r round(100*filter(feed_summary, feed == "future", macro == "P")$digest, 1)`% digestible.
The digestibility of the other macronutrients are assumed to remain constant across all ingredients at `r round(100*filter(feed_summary, feed == "past", macro == "C")$digest, 1)`% for carbohydrates and `r round(100*filter(feed_summary, feed == "past", macro == "L")$digest, 1)`% for lipids. 
