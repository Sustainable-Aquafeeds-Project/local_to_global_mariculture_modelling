...
[This intro is copied verbatim from work done during one of the early workshops.]{.aside}
...
[Unfinished. Need to work out how much should be transposed from @baldan_r_2018. One thing that definitely needs to be stated is the individual ingredient (rather than whole food) tracking, the use of cohorts, and the new method for calculating population.]{.aside}
...
Farms with a mean temperature of $\leq 6^\circ$C were excluded (`r as.integer(length(farms_to_omit))` farms). The total number of farms included in this analysis was therefore `r nrow(farm_temp_means)`.
...
* Salmon cohorts were initialised Spring. This was on day `r unique(farm_coords$t_start[farm_coords$lat < 0])` for farms in the southern hemisphere and day `r unique(farm_coords$t_start[farm_coords$lat > 0])` for farms in the northern hemisphere.
...
* Within the reference feed, protein is overall `r round(100*digestP_1,1)`% digestible, lipid is overall `r round(100*digestL,1)`% digestible, and carbohydrate is overall `r round(100*digestC_1,1)`% digestible.
* Within the reference feed, protein is overall `r round(100*digestP_2,1)`% digestible, lipid is overall `r round(100*digestL,1)`% digestible, and carbohydrate is overall `r round(100*digestC_2,1)`% digestible.
* Within the reference feed, protein is overall `r round(100*digestP_3,1)`% digestible, lipid is overall `r round(100*digestL,1)`% digestible, and carbohydrate is overall `r round(100*digestC_3,1)`% digestible.
...
@fig-harvest-size shows the distribution of average individual harvest sizes across farms used to create the population timeseries as predicted by the fish-growth model. 
