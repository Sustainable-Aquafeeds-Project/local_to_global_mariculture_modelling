...
farm_IDs <- tar_read(farm_IDs, store = indi_store)
...
sst <- here() %>% file.path("data", "_general_data", "SST", "farm_SST_extracted.parquet") %>% read_parquet()

# This is the actual temperature curve for the farming period
farm_temp <- tar_read(example_individual, store = indi_store, branches = b1)
```
...
pop_params <- tar_read(pop_params, store = indi_store)
weight <- tar_read(example_individual, store = indi_store, branches = b) %>% select(farm_ID, weight, days, prod_days, feed)
SGR <- tar_read(example_individual, store = indi_store, branches = b) %>% select(farm_ID, SGR, days, prod_days, feed)
FCR <- tar_read(example_individual, store = indi_store, branches = b) %>% select(farm_ID, FCR, days, prod_days, feed)
...
df <- data.frame(water_temp = seq(0, 30, 0.25))
...
feed_params_protein <- tar_read(feed_params_protein, store = indi_store)
feed_params_carbs <- tar_read(feed_params_carbs, store = indi_store)
feed_params_lipids <- tar_read(feed_params_lipids, store = indi_store)
```
...
#c(b1, b2, b3))
ing_pot <- tar_read(example_individual, store = indi_store, branches = b1) %>% select(farm_ID, ing_pot, days, prod_days, feed)
ing_act <- tar_read(example_individual, store = indi_store, branches = b1) %>% select(farm_ID, ing_act, days, prod_days, feed)
food_prov <- tar_read(example_individual, store = indi_store, branches = b1) %>% select(farm_ID, food_prov, days, prod_days, feed)
```
...
params <- tar_read(sens_all_params, store = indi_store)
pnames <- names(params)
```
