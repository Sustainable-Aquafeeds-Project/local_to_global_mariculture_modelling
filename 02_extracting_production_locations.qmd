...
Clean aquaculture production data from the FAO and isolate production data for Atlantic salmon.
...
# Apply species codes and filter by relevant species groups
species_codes <- file.path(rawdata_path, "Aquaculture_2025.1.0", "CL_FI_SPECIES_GROUPS.csv") %>% 
  read.csv() %>% 
  filter(str_detect(Scientific_Name, "Salmo")) %>% 
  filter(Scientific_Name %in% c("Salmoniformes (=Salmonoidei)", "Salmo spp", "Salmo salar", "Salmonidae")) %>% 
...
# Apply status codes and filter by relevant species groups
