...
here("src") %>% list.files(pattern = "\\.R$", full.names = TRUE) %>% str_subset("map", negate = T) %>% walk(source)
```
...
test_reference_feed # should be ~1430 g, or really anything other than 0
...
targets::tar_make(
  names = !c("farm_run", "cohort_results", "biomass_produced", "farm_results"), 
...
targets::tar_load(biomass_produced)
qsave(biomass_produced, file.path(output_model_farm_path, str_c("biomass_produced_all_farms.qs")))
```
