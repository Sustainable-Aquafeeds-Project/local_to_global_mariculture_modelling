...
Note that this takes a long time - each branch of farmrun takes ~5 minutes, and a maximum of ~12 workers can run at a time. The total therefore takes ~17 hours.
...
purrr::map(stat_names, function(stat_name) {
  farm_fnms %>% 
    ...
    mutate(
      ...
      sd_perbiom = (sd_mean+biom_sd_mean) * mean_perbiom
    ) %>% 
